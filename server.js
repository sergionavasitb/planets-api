...
// ─────────────────────────────────────────
// HELPER — construir URLs de imágenes locales
// ─────────────────────────────────────────
const img = (req, file) => `${baseUrl(req)}/imagenes/${file}`;
...
const bodiesData = {
  ...
  "096": {
    ...
    image_file: "proxima_centauri_b.jpg",
  ...
  "113": {
    ...
    image_file: "Parker_Solar_Probe.jpg",
  ...
  "117": {
    ...
    image_file: "estrella-de-la-muerte.jpg",
  ...
  "119": {
    ...
    image_file: "estacion_espacial_2001.jpg",
  ...
  "085": {
    ...
    image_file: "MilkyWay_chocolatina.jpg",
...
app.get("/", (req, res) => {
  ...
  res.json({
    ...
    endpoints: {
      ...
      "GET /v1/bodies/:id/image":        "Redirige a la imagen JPG local",
...
app.get("/v1/bodies", (req, res) => {
  ...
  res.json({
    ...
    bodies: result.map(b => ({
      ...
      type: b.type,
      image_url: b.image_file ? `${base}/imagenes/${b.image_file}` : null,
...
app.get("/v1/bodies/:id", (req, res) => {
  ...
  res.json({
    ...body,
    image_url: body.image_file ? `${base}/imagenes/${body.image_file}` : null,
    model_url: `${base}/models/${body.glb_file}`,
  });
});
...
// GET /v1/bodies/:id/image
app.get("/v1/bodies/:id/image", (req, res) => {
  const id = req.params.id.padStart(3, "0");
  const body = bodiesData[id];
  if (!body) return res.status(404).json({ error: "Cuerpo no encontrado" });
  if (!body.image_file) return res.status(404).json({ error: "Este cuerpo no tiene imagen local" });
  res.redirect(`/imagenes/${body.image_file}`);
});
...
app.get("/v1/planets", (req, res) => {
  ...
  res.json({
    ...
    planets: result.map(p => ({
      ...
      rings: p.rings,
      image_url: p.image_file ? `${base}/imagenes/${p.image_file}` : null,
...
app.get("/v1/planets/:id", (req, res) => {
  ...
  res.json({
    ...planet,
    image_url: planet.image_file ? `${base}/imagenes/${planet.image_file}` : null,
    model_url: `${base}/models/${planet.glb_file}`,
  });
});
...
app.get("/v1/compare", (req, res) => {
  ...
  res.json({
    ...
    images: {
      [bodyA.name]: bodyA.image_file ? `${base}/imagenes/${bodyA.image_file}` : null,
      [bodyB.name]: bodyB.image_file ? `${base}/imagenes/${bodyB.image_file}` : null,
    },
    models: {
...
app.get("/v1/fact/:id", (req, res) => {
  ...
  res.json({
    ...
    fun_fact: body.fun_fact || "Sin curiosidad registrada.",
    image_url: body.image_file ? `${baseUrl(req)}/imagenes/${body.image_file}` : null,
  });
});
